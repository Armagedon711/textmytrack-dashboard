...
import { 
  ...
  Lock, Ban, Trash2, Plus, ShieldAlert, User, Music, Settings, Link as LinkIcon 
} from "lucide-react";
...
export default function SettingsModal({ 
  ...
  const [tipLink, setTipLink] = useState(djProfile?.tip_link || "");
  const [reviewLink, setReviewLink] = useState(djProfile?.review_link || "");
  ...
  useEffect(() => {
    if (djProfile) {
        ...
        setTipLink(djProfile.tip_link || "");
        setReviewLink(djProfile.review_link || "");
    }
  }, [djProfile]);
       ...
       <div className="relative w-full max-w-4xl bg-[#12121a] rounded-2xl border border-white/10 shadow-2xl overflow-hidden flex flex-col md:flex-row h-[90vh] md:h-[80vh] max-h-[90vh] md:max-h-[85vh]">
         ...
         <div className="w-full md:w-64 bg-[#16161f] border-b md:border-b-0 md:border-r border-white/5 flex flex-col shrink-0">
            ...
            <div className="flex md:flex-col overflow-x-auto md:overflow-visible px-4 md:px-2 space-x-2 md:space-x-0 md:space-y-1 pb-4 md:pb-0 scrollbar-hide">
              ...
              <NavItem id="links" label="Links & Tips" icon={LinkIcon} activeSection={activeSection} setActiveSection={setActiveSection} />
         ...
         <div className="flex-1 flex flex-col relative bg-[#12121a] min-h-0">
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 md:p-8">
                ...
                {activeSection === 'links' && (
                    <div className="space-y-8 max-w-lg">
                        <div>
                            <h3 className="text-xl font-bold text-white mb-1">Links & Tips</h3>
                            <p className="text-gray-500 text-sm">Links to send guests at the end of the night.</p>
                        </div>

                        <div className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-300">Tip Link (Venmo/CashApp)</label>
                                <div className="relative">
                                   <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                   ...
                                   <input 
                                       ...
                                       className="w-full bg-[#1b1b2e] border border-[#2a2a40] rounded-xl pl-10 pr-4 py-3 text-white outline-none focus:border-pink-500 transition-all" 
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-300">Review Link</label>
                                <div className="relative">
                                   <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            ...
                            <p className="text-xs text-gray-500 bg-white/5 p-3 rounded-lg border border-white/5">
                                💡 <strong>Pro Tip:</strong> When you finish your set and toggle "Accepting Requests" to OFF, we'll ask if you want to text these links to everyone who made a request tonight.
                            </p>
                        </div>
                    </div>
                )}
                        ...
                        <div className="p-6 bg-[#1b1b2e]/50 border border-white/5 rounded-2xl space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input type="number" min="1" max="100" value={limitCount} onChange={(e) => setLimitCount(e.target.value)} className="w-full bg-[#0e0e14] border border-[#2a2a40] rounded-xl px-4 py-3 text-white text-lg font-mono outline-none focus:border-pink-500 transition-colors" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Time Window (Hours)</label>
                                    <input type="number" min="1" max="24" value={limitHours} onChange={(e) => setLimitHours(e.target.value)} className="w-full bg-[#0e0e14] border border-[#2a2a40] rounded-xl px-4 py-3 text-white text-lg font-mono outline-none focus:border-pink-500 transition-colors" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {activeSection === 'blacklist' && (
                    <div className="space-y-6 h-full flex flex-col">
                        ...
                        <div className="flex gap-2">
                             <input value={newBanNumber} onChange={(e) => setNewBanNumber(e.target.value)} className="flex-1 min-w-0 bg-[#1b1b2e] border border-[#2a2a40] rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:border-red-500 outline-none transition-all" placeholder="Enter phone number..." />
                             <button onClick={handleBan} className="bg-red-500/10 text-red-400 border border-red-500/20 px-4 md:px-6 py-2 rounded-xl hover:bg-red-500/20 font-medium transition-colors flex items-center gap-2 flex-shrink-0"><Ban size={18} /> Block</button>
                        </div>
                                ...
                                {blacklist.map((item) => (
                                    <div key={item.id} className="p-3 flex items-center justify-between group hover:bg-white/5 rounded-lg transition-colors">
                                        <div className="flex items-center gap-3 overflow-hidden">
                                            <div className="w-8 h-8 rounded-full bg-red-500/10 flex items-center justify-center flex-shrink-0"><Ban size={14} className="text-red-500" /></div>
                                            <span className="text-gray-300 font-mono truncate">{formatPhoneNumber(item.phone_number)}</span>
                                        </div>
                                        <button onClick={() => handleUnban(item.phone_number)} className="p-2 hover:bg-white/10 rounded-lg text-gray-500 hover:text-green-400 transition-colors flex-shrink-0"><Trash2 size={16} /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-[#1b1b2e]/50 border border-white/5 rounded-2xl overflow-hidden divide-y divide-white/5">
                            <div className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                                <div className="flex items-center gap-4 text-gray-400">
                                    ...
                                    <div><p className="text-sm font-medium text-white">Email Address</p><p className="text-xs text-gray-500">{user?.email}</p></div>
                                </div>
                            </div>
                            <div className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                                <div className="flex items-center gap-4 text-gray-400">
                                    <div className="p-2 bg-white/5 rounded-lg"><Lock size={20} /></div>
                                    <div><p className="text-sm font-medium text-white">Password</p><p className="text-xs text-gray-500">Last changed recently</p></div>
                                </div>
                                <button onClick={handleResetPassword} disabled={resetLoading} className="text-xs bg-white/5 hover:bg-white/10 text-white px-4 py-2 rounded-lg border border-white/5 font-medium transition-colors w-full md:w-auto">{resetLoading ? "Sending..." : "Reset Password"}</button>
                            </div>
                        </div>
                    </div>
                )}
            ...
            {/* SAVE FOOTER */}
            {(activeSection === 'profile' || activeSection === 'controls' || activeSection === 'links') && (
                <div className="p-4 border-t border-white/5 bg-[#16161f] flex justify-end">
                    <button onClick={handleSaveSettings} disabled={loading} className="bg-pink-600 hover:bg-pink-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-pink-900/20 transition-all flex items-center gap-2 w-full md:w-auto justify-center">
                        {loading ? <span className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/> Saving...</span> : <><Save size={18} /> Save Changes</>}
                    </button>
                </div>
            )}
         </div>
